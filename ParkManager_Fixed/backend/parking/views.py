...
def login_view(request):
    ...
    print(f"Login attempt: car_number={car_number}, role={role}, password={password}")

    # First, find user by car_number
    try:
        user = User.objects.get(car_number=car_number)
        print(f"User found: {user.car_number}, role={user.role}")
    ...
    # Check role
    if user.role != role:
        print(f"Role mismatch: user is {user.role}, expected {role}")
        return Response({'error': f'This account is not a {role} account'}, status=403)

    print("Login success, calling django login()")
...
def user_dashboard(request):
    ...
    data = {
        ...
        'subscription_plan': getattr(user.subscription, 'plan', 'None') if hasattr(user, 'subscription') else 'None',
...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Get the slot object
        slot = serializer.validated_data['slot_code']
...
class StaffViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        department = request.data.get('department', 'Parking Management')
        is_on_duty = request.data.get('is_on_duty', False)
        
        if not employee_id or not name:
            return Response({'error': 'Employee ID and name are required'}, status=400)
        
        # Check if user already exists
        if User.objects.filter(username=employee_id).exists():
            return Response({'error': 'Staff with this employee ID already exists'}, status=400)
...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def broadcast(self, request):
        ...
        notifications = [Notification(user=u, notification_type='System', title=title, message=message) for u in users]
...
class LostFoundViewSet(viewsets.ModelViewSet):
    queryset = LostFound.objects.all().select_related('reported_by', 'found_by', 'claimed_by')
